...
#' @return output a dataframe formats with miRNA names with the number of bridging ceRNA triplets and the corresponding genes with the number of ceRNA triplets.
#'
...
miroutput=matrix(0,length(unlist(summary_miR_2[,1])),2)
j=1
for (i in 1:4)
{
miroutput[c(j:(j-1+length(summary_miR_2[,1][i][[1]]))),2]=summary_miR_2[i,2][[1]]
for (p in (j:(j-1+length(summary_miR_2[,1][i][[1]])))){
miroutput[p,1]=unlist(summary_miR_2[,1])[p]
}
j=length(summary_miR_2[,1][i][[1]])+j
}

cn=min(length(miroutput[,1]),length(hubmiR[,1]))
output=cbind(miroutput[(1:cn),],hubmiR[(1:cn),])
colnames(output)=c("miRNA","bridging ceRNA triplets","Gene","ceRNA triplets")

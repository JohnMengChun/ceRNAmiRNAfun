...
segcluster_peakmerge=function(cor_shreshold_peak,dictionary,miRNA_total,mirna_sam,gene_sam,Realdata){
  no_cores <- 1   #detectCores()/6
  cl<-makeCluster(no_cores,outfile="") #outfile: so it can print()
  ...
  datap <- foreach(mir=miRNA_total)  %dopar%  {
    ...
    Gene <- unlist(lapply(Gene, function(k) if(identical(k, character(0)))NULL else k))#increased coding
    ...
    tmp <- tryCatch({
      ...
      k=foreach(p=1:total_pairs, .combine = "rbind")  %dopar%  {
        ...
        if(sum(is.na(triplet$corr)) ==0){
          ...
          colnames(CNA.object) <- c("chrom","maploc",paste("gene",r,"and",s))  ### gene name
